    ...
    <div class="row">
        <div class="col-lg-9">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" class="needs-validation" action="{{ route('teams.update', $team->id) }}" enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('PUT')
                        <div class="row mb-3">
                            <div class="col-md-12">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">                                
                                ...
                                <select 
                                    ...
                                    required
                                >
                                    <option value="{{ $team->leader_id }}">{{ $team->leader->first_name . ' ' . $team->leader->last_name }}</option>
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}">{{ $user->first_name . ' ' . $user->last_name }}</option>
                                    @endforeach
                                </select>
...
@section('script')
    ...
    <script>
        ...
        let selector = new Selectr('#members',{
            multiple: true,
            placeholder: 'Select Team Members...'
        });

        selector.setValue(selectedMemberss)
    </script>
@endsection
